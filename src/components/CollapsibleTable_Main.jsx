  ...
  return (
    <React.Fragment>
      <TableRow sx={{ '& > *': { borderBottom: 'unset' } }} >
        ...
        <TableCell align="left"><Img.Main MainI={row.name} /></TableCell>
...
export function Main(props) {
  ...
  const rows = [//name,St
    createData( props.SlctMain, props.Main),
  ];
  ...
  return (
    <TableContainer component={Paper}>
      <Table aria-label="collapsible table" size="small" >
        <TableHead>
          <TableRow>
            <TableCell align="center" colSpan={3}>メインキャラ</TableCell>
          </TableRow>
        </TableHead>
        <TableBody>
          {rows.map((row) => (
            <Row key={row.name} row={row} Main={props.Main} SlctSprt={props.SlctSprt}/>
          ))}
        </TableBody>
      </Table>
    </TableContainer>
